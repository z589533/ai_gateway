...
- 管理后台用户体系 / RBAC 登录（若实现 Dashboard，MVP **免登录**，见 §18）
- 计费、发票、多区域部署
...
| 测试说明 | `go test ./...`、模块级单元测试覆盖点、Dashboard 选做测试命令 |
...
| 16 | 管理后台免登录可用（选做） | 打开 http://localhost:8848 直达租户/Key/用量页 |
...
### 18.4 API 封装（`src/api/gateway.ts`）
...
*文档版本：v1.3 | 补齐每个模块的单元测试设计与测试验收要求*
