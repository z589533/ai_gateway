...
- P1 增强：管理后台已实现为静态 HTML/CSS/JS + nginx，随 `docker compose up --build` 启动。

### 1.2 不在 MVP 范围（已知限制）

- 真实上游 LLM 对接（用 mock 代替）
- 多副本分布式限流一致性（单机 sentinel 足够）
- 管理后台用户体系 / RBAC 登录（Dashboard **免登录**，见 §18）
- 计费、发票、多区域部署
...
| 管理后台 | 静态 HTML/CSS/JS + nginx | 租户、Key、用量、代理测试；零构建，compose 直接启动 |
...
```
  浏览器 ──► Static Admin (web/)  免登录，直达管理页
       │              │
...
├── web/                          # 静态管理后台（免登录）
│   ├── assets/
│   │   ├── app.js                # API 调用、状态管理、页面交互
│   │   └── styles.css            # 管理台样式
│   ├── test/
│   │   └── admin-ui.test.mjs     # 静态 UI 单元测试
│   ├── index.html                # 租户、Key、用量、代理测试页面
...
| `ADMIN_DASHBOARD_PORT` | `8848` | compose 中管理后台对外端口（文档变量，实际由 compose ports 控制） |
...
默认 `docker compose up --build` 启动 `mysql`、`redis`、`app`、`admin`。访问管理后台：http://localhost:8848（打开即用，无需登录）。
...
| 测试说明 | `go test ./...`、模块级单元测试覆盖点、`cd web && npm test` |
| 已知限制 | mock、非流式、估算 token、单机限流、管理 Token、Dashboard localStorage Token |
...
### 14.2 P1 Dashboard（已实现）

| 阶段 | 任务 | 预估 |
|------|------|------|
| P1 | 静态 Dashboard 页面结构与样式 | 35min |
| P1 | 租户 / Key / 用量 / 代理测试交互 | 55min |
| P1 | Dashboard compose/nginx 联调与测试 | 20min |

Dashboard 是需求中的选做项，当前实现为零构建静态管理台，默认随 compose 启动。
...
| 模块 | 测试文件 | 单元测试重点 |
|------|----------|--------------|
| `web/src/api/gateway.ts` | `gateway.test.ts` | API 路径、参数、Authorization Header |
| `web/src/router/modules/gateway.ts` | `gateway-route.test.ts` | 三个菜单路由存在，默认首页指向租户页 |
| `web/src/views/tenant` | `tenant.spec.ts` | 列表加载、新建租户、启用/禁用交互 |
| `web/src/views/apikey` | `apikey.spec.ts` | 创建 Key 后展示 `secret_key`，关闭后不再保留明文 |
| `web/src/views/usage` | `usage.spec.ts` | 查询条件、summary 展示、空数据状态 |
...
## 18. 管理后台：静态 Dashboard（免登录）

### 18.1 选型说明

采用静态 HTML/CSS/JavaScript + nginx 托管，不引入前端构建链。

| 项 | 说明 |
|----|------|
| 技术 | 原生 HTML/CSS/JS、fetch、nginx |
| 优势 | 零构建、镜像小、启动快，适合面试 MVP 和 `docker compose up --build` |
| 入口 | `web/index.html` |
| 样式 | `web/assets/styles.css` |
| 交互 | `web/assets/app.js` |
| 测试 | `cd web && npm test` |

**Trade-off：** 放弃大型后台框架的内置组件，换取更低复杂度和更稳定的一键启动；页面复杂度在租户、Key、用量、代理测试范围内仍可维护。

### 18.2 免登录方案（MVP 核心决策）

管理台不做用户登录，页面顶部可配置：

- API Base：compose/nginx 场景默认留空，由 nginx 反代 `/api` 和 `/v1` 到后端 `app:8080`；直接打开 HTML 时可填 `http://localhost:8080`。
...
| 菜单 | 路由 | 功能 |
|------|------|------|
| 概览 | `overview` view | 租户数、Key 数、Token 汇总、错误数、最近用量 |
| 租户管理 | `tenants` view | 列表、新建、启用/禁用 |
| API Key | `keys` view | 按租户筛选；创建 Key；一次性展示 `secret_key`；编辑 scope / 状态 / 过期时间；软删除 |
| 用量统计 | `usage` view | 按租户、Key、时间范围查询；明细表格 + summary 汇总 |
| 代理测试 | `playground` view | 输入 API Key Secret 后调用 `/v1/models` 与 `/v1/chat/completions` |

### 18.4 API 封装

前端使用两个 fetch 包装：

- `adminFetch(path, options)`：调用 `/api/v1/*`，自动附带管理 Token，解析 `{ code, message, data }`。
- `gatewayFetch(path, secret, options)`：调用 `/v1/*`，使用调用方 API Key Secret，展示 OpenAI 风格响应或错误。
...
也可以直接打开 `web/index.html`，并在页面顶部将 API Base 设置为 `http://localhost:8080`。

### 18.6 nginx 反代

`Dockerfile.web` 使用 nginx 托管静态文件，`web/nginx.conf` 将以下路径反代到后端：
...
*文档版本：v1.4 | 实现静态管理后台并纳入 docker compose 默认启动*
